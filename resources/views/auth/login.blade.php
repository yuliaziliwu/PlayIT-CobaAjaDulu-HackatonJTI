...
<body class="flex items-center justify-center min-h-screen">
    <div class="flex justify-between bg-white rounded-lg overflow-hidden">
        ...
        <div class="p-8 w-full md:w-1/2">
            <h2 class="text-2xl font-bold text-orange-600 text-center mb-2">Log In</h2>
            <form action="{{ route('authenticate') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="email" class="block text-gray-700 text-sm">Email</label>
                    <input type="text" id="email" name="email" class="w-full px-3 py-1.5 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-orange-500" required>
                </div>
                <div class="mb-3">
